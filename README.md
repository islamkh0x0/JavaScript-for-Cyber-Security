# JavaScript for Cyber Security

This repository contains notes on JavaScript concepts relevant to penetration testers and bug hunters. It serves as a reference for myself and others who want to learn JavaScript for cybersecurity purposes.

---
## Table of Contents

### 1️⃣ JavaScript Basics
- [What is JavaScript?](what-is-JavaScript.md)  
- [JavaScript Engine](JavaScript-Engine.md)  
- [Hello World in JavaScript](Hello-world.md)  
- [JavaScript Comments](JS-Comments.md)  
- [JavaScript Variables](Variables.md)  
- [JavaScript Data Types](Data-Types.md)

### 2️⃣ Core Operations & Data Handling
- [Basic Operators](Basic-Operators.md)  
- [Type Conversions](Type-Conversions.md)  
- [User Interaction "Popup Boxes"](User-Interaction.md)  

### 3️⃣ Control Flow
- [Conditional Statements](Conditional-Statements.md)  
- [Logical Operators](Logical-Operators.md)  
- [Nullish Coalescing Operator](Nullish-Coalescing.md)  
- [Switch-Case Statement](Switch-Case.md)  

### 4️⃣ Loops
- [Loops: while, for](Loops.md)  

### 5️⃣ Functions
- [JavaScript Functions](Functions.md)  

### 6️⃣ Strict Mode & Code Quality
- ["use strict" Mode](Use-Strict.md)  

---
---
